function [Tl,contributionsTL]=TouschekPiwinskiLifeTime_inputoptics(r,dpp,Ib,positions,varargin)
...
[lo,~]=getoption(varargin, 'lo', []);
[pa,~]=getoption(varargin, 'pa', []);

if isempty(lo) || isempty(pa)
    [lo2,pa2]=atx(r,0,positions);
    if isempty(lo)
        lo = lo2;
    end
    if isempty(pa)
...
[emitx,~]=getoption(varargin, 'emitx', pa.modemittance(1));
[emity,~]=getoption(varargin, 'emity', pa.modemittance(2));

[sigp,~]=getoption(varargin, 'espread', pa.espread);
[sigs,~]=getoption(varargin, 'blength', pa.blength);
